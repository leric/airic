...
1. ACP 客户端 open/focus 一个 markdown 文件 → `OpenDocumentUseCase` 设置 `session.currentDocument`；或 agent 调用 `document.focus` 工具——二者共用 `setActiveDocument` service。
...
- 每轮取 `root → currentTurnId` 路径上的节点，投影为 `TranscriptMessage[]`。
- 若 turn 有 `toolTrace`，投影时使用**与 current turn 相同的消息结构**（`user` / `assistant`+`toolCalls` / `tool_result`），但 `tool_result.content` 由 `compactToolTraceForProjection()` 替换为 `{ACP title} — ok|failed|rejected` 短提示，不含 read 正文、grep 匹配、diff 详情等。
- 若 turn 无 `toolTrace`，仍只投影 `userMessage` + `assistantMessage` 文本对。
- **被排除**：兄弟分支；完整 tool 返回详情（存于 `TurnNode.toolTrace` 供 ACP replay，LLM 只见压缩版）。
- Summary 替换 range 时：被压缩 turn 的对话文本由 summary 顶替，但其 compact toolTrace（去掉 leading user）仍按 turn 顺序拼接在 summary 之前，避免行动路径丢失。
- [history-tree.md](history-tree.md) 描述的 summarize / cursor / mark 都在这条通道里做，不影响 system prompt 通道。
...
8. **Anti-pollution history**：history 永远是 cursor path 投影，不是平铺全量；prior-turn tool 返回详情不进 context，但 compact toolTrace（同构消息 + 短 result 提示）进 context，保留行动路径。
