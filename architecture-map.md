...
- Session history → turn tree in `domain/session/turn-tree.ts`; model context uses `projectCursorPath()` (active cursor path only; raw digression and `toolTrace` excluded after `/sumup`). System prompt only → `RuntimeContextBuilder` (base instruction + active mode spec + process index or active process spec + always-resident Tool Usage + current document).
