...
1. **Reconstruct the actual process.** Describe what the agent did, not what it should have done — where it looked, what it tried, where it was corrected, where things flowed. The conversation path includes compact prior-turn tool traces (same message shape as the current turn, with tool results shortened to title + status); use those records before re-reading workspace files. Do not go looking for session logs, transcript files, or `.airic/logs/` to reconstruct history when the cursor-path projection already shows the action sequence. This is observation, recall, and reflection, not new work or open-ended retrieval.
